...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f <$> (StateT g) = StateT (\ s -> (\ (x,y) -> (f x, y)) <$> g s)
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
  (StateT g) <*> (StateT h) = StateT (\ s ->  g s >>= (\ (x, s') -> (\ (y, s'') -> (x y, s'') ) <$> (h s')))
--TODO the <*> seems implemented incorrectly
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  mf =<< (StateT g) = StateT (\ s -> g s >>= (\ (x, s') -> runStateT (mf x) s') )
...
state' ::
  (s -> (a, s))
  -> State' s a
state' g = StateT (\ s -> ExactlyOne (g s))
...
execT ::
  ...
  -> s
  -> f s
execT state = ((<$>) P.snd)  . runStateT state
...
evalT ::
  ...
  -> s
  -> f a
evalT state = ((<$>) P.fst)  . runStateT state
...
putT ::
  ...
  s
  -> StateT s f ()
putT inital = StateT (\ _ -> pure ((), inital))
...
instance Monad f => Monad (OptionalT f) where
  mf =<< (OptionalT g) = OptionalT (g >>= (\ x -> _help x mf))

_help :: Monad f => Optional a -> (a -> OptionalT f b) ->  f (Optional b)
_help Empty _    =  pure Empty
_help (Full a) g = runOptionalT (g a)
...
instance Monad (Logger l) where
  mf =<< Logger l' a = let Logger l'' b = mf a in Logger (l' ++ l'') b
...
distinctG xs = runOptionalT  $ evalT ( filtering helper'' xs) S.empty
      where helper'' :: (Integral a, Show a) => a -> StateT (S.Set a)  (OptionalT (Logger Chars)) Bool
            helper'' = (\ a -> StateT (\ s -> if a > 100 then OptionalT $ log1 ("aborting > 100: " ++ (show' a)) Empty
                                                         else if even a then OptionalT $ log1 ("even number: " ++ show' a) (Full (S.notMember a s, S.insert a s))
                                                                         else OptionalT $ Logger (Nil) (Full (S.notMember a s, S.insert a s)  )))
